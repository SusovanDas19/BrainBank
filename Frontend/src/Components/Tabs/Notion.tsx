...
import { NotionSharedSelector } from "../../store/selectors/shareBrainSelector";
import { NoContent } from "../UI/NoContent";

export const Notion = ({isShare}:{isShare: boolean}) => {
  ...
  const ShareNotion = useRecoilValue(NotionSharedSelector);

  useEffect(() => {
    setCurrTab("Notion");
    setSelectedOption("Notion");

    if(isShare){
      setAllData(ShareNotion);
      setLoading(false)
      return;
    }
    ...
    const getData = async () => {
      try {
        ...
        const url =
          `https://api.brainbank.cv/v1/content/fetch?type=Notion` +
  ...
  return (
    <div className="h-screen flex flex-col bg-white dark:bg-primaryBlack">
      {loading ? (
        ...
        <div className="flex-1 overflow-y-auto pt-46 p-4 top-30 pb-10 ml-10 mr-0 md:ml-10 md:mr-10 mt-10">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 w-fit justify-center items-center">
